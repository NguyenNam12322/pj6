...
class AjaxController extends Controller
{
    ...
    public function getProductActive(Request $request)
    {
        ...
        if(!empty($search)){
            ...
            if(!Cache::has('product_search')){
                ...
                $productss = product::select('Link', 'Name', 'Image', 'Price', 'id', 'ProductSku', 'promotion',  'Salient_Features')->where('active', 1)->get();
            ...
            foreach($product as $products){
                ...
                $sugest = '<li class="sugests-li">
                            ...
                            <a class="suggest_link" href="'.route('details', $products->Link).'"><p> '. _substrs(str_replace(['Máy lọc nước', 'Máy Lọc Nước'], 'MLN', $products->Name),50).'</p>
    ...
    public function showGroupData(Request $request)
    {
        ...
        $data = product::whereIn('id', $hot->toArray())->Orderby('orders_hot', 'desc')->take(16)->get();
    ...
    public function addProductToCart(Request $request)
    {
        ...
        Cart::add(['id' => $id, 'name' => $data_Product->Name,  'qty' => 1, 'price' => $price, 'weight' => '500',  'options' => ['link' => $data_Product->Link, 'gift'=>$gift??'', 'transportation_cost'=> !empty($newPrice[0])?intval($newPrice[0])- intval($price):0]]);
